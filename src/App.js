...
function App() {
  ...
  const valueChange = (event) => {
    ...
    if (!Number.isNaN(+event.target.value[event.target.value.length - 1])) {
      setValue(state.musk(event.target.value))
    }
    ...
    <div className="App" >
      ...
      <Modal active={ModalActive}
        ...
        <div className={modal.modal_header}>
          ...
          <Button type='text'
            onClick={() => setModalActive(false)} text={
        ...
        <p className={modal.modal_descr}>Используйте налоговый вычет чтобы погасить ипотеку досрочно. Размер налогового вычета составляет не более 13% от своего официального годового дохода.</p>
        <div className={modal.modal_paycheck}>
          <span>Ваша зарплата в мессяц</span>
          <Input error={Error} value={Value} onChange={valueChange} placeholder='Введите данные' />
          <Button type='text' text='Расчитать' onClick={calculate} />
        </div>
        <div className={modal.modal_choice}>
          <span>Что уменьшаем?</span>
          <Radio name='modal_radio'
            id='payments'
            for='payments'
            label='Платеж' />
          <Radio name='modal_radio'
        ...
        <div>
          {
            state.pays.length > 0 ?
              <span>Итого можете внести в качестве досрочных:</span> : null
          }
          <Payments />
        </div>
        <Button style={{ width: '100%' }} type='contained' text='Добавить' />
      </Modal >
